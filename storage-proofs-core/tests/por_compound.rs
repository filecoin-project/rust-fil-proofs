...
fn por_compound<Tree: 'static + MerkleTreeTrait>() {
    ...
    let verifying_key =
        PoRCompound::<Tree>::verifying_key::<XorShiftRng>(None, &public_params.vanilla_params)
    ...
    let verified = PoRCompound::<Tree>::verify(
        &public_params,
        &public_inputs,
        &multi_proof,
        &NoRequirements,
...
fn por_compound_private_root<Tree: 'static + MerkleTreeTrait>() {
    ...
    for i in 0..3 {
        ...
        // Don't try to generate the groth parameters, as they should already have been generated
        // by the `groth_params()` call above.
        let verifying_key =
            PoRCompound::<Tree>::verifying_key::<XorShiftRng>(None, &public_params.vanilla_params)
                .expect("failed to get veriyfing key");
        let prepared_verifying_key = groth16::prepare_verifying_key(&verifying_key);
        ...
        let verified = PoRCompound::verify(
            &public_params,
            &public_inputs,
            &multi_proof,
            &NoRequirements,
