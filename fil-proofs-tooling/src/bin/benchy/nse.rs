...
use storage_proofs::merkle::{DiskTree, MerkleTreeTrait};
use storage_proofs::porep::nse::{
    self, ChallengeRequirements, Config, NarrowStackedExpander, PrivateInputs, PublicInputs,
    SetupParams, TemporaryAuxCache,
};
...
fn dump_proof_bytes<Tree: MerkleTreeTrait>(
    all_partition_proofs: &[Vec<nse::Proof<Tree, Sha256Hasher>>],
) -> anyhow::Result<()> {
...
impl From<Params> for Inputs {
    fn from(p: Params) -> Self {
        Inputs {
            ...
            degree_expander: p.degree_expander,
            degree_butterfly: p.degree_butterfly,
            num_expander_layers: p.num_expander_layers,
            num_butterfly_layers: p.num_butterfly_layers,
            num_challenges_window: p.num_challenges_window,
...
fn generate_report<Tree: 'static>(params: Params, cache_dir: &TempDir) -> anyhow::Result<Report>
where
    Tree: MerkleTreeTrait,
{
    let FuncMeasurement {
        ...
        let replica_id = <Tree::Hasher as Hasher>::Domain::random(rng);
        let config = Config {
            k: 4,
            num_nodes_window: nodes,
            degree_expander: *degree_expander,
            degree_butterfly: *degree_butterfly,
            num_expander_layers: *num_expander_layers,
            num_butterfly_layers: *num_butterfly_layers,
            sector_size: *data_size,
        };
        let sp = SetupParams {
            config: config.clone(),
            num_challenges_window: *num_challenges_window,
        };
        ...
        let (pub_in, priv_in, d) = if *bench_only {
            ...
            let FuncMeasurement {
                ...
                let t_aux =
                    TemporaryAuxCache::<Tree, Sha256Hasher>::new(&config, &t_aux, replica_path)
                ...
                let proofs_are_valid =
                    NarrowStackedExpander::<Tree, Sha256Hasher>::verify_all_partitions(
                        &pp,
                        &pub_inputs,
        ...
        if let Some(data) = d {
            if *extract {
                let m = measure(|| {
                    NarrowStackedExpander::<Tree, Sha256Hasher>::extract_all(
                        &pp,
                        &replica_id,
                        &data,
...
pub fn run(opts: RunOpts) -> anyhow::Result<()> {
    let params = Params {
        ...
        degree_expander: opts.degree_expander,
        degree_butterfly: opts.degree_butterfly,
        num_expander_layers: opts.num_expander_layers,
        num_butterfly_layers: opts.num_butterfly_layers,
        num_challenges_window: opts.num_challenges_window,
        use_tmp: !opts.no_tmp,
    ...
    info!("Benchy NSE: {:?}", &params);

    let cache_dir = tempfile::tempdir().unwrap();

    let report = match params.hasher.as_ref() {
        "poseidon" => generate_report::<DiskTree<PoseidonHasher, U8, U2, U0>>(params, &cache_dir)?,
        "pedersen" => generate_report::<DiskTree<PedersenHasher, U8, U8, U0>>(params, &cache_dir)?,
        "sha256" => generate_report::<DiskTree<Sha256Hasher, U8, U8, U0>>(params, &cache_dir)?,
        "blake2s" => generate_report::<DiskTree<Blake2sHasher, U8, U8, U0>>(params, &cache_dir)?,
