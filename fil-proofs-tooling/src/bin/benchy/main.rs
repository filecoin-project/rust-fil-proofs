...
fn main() -> Result<()> {
    ...
    let nse_cmd = SubCommand::with_name("nse")
                .about("Run nse sealing")
                .arg(
                    Arg::with_name("size")
                        .required(true)
                        .long("size")
                ...
                .arg(
                    Arg::with_name("num_challenges_window")
                        .long("challenges")
                        .help("How many challenges to execute per window")
                        .default_value("2")
                        .takes_value(true),
                )
                .arg(
                    Arg::with_name("degree_expander")
                        .long("degree-expander")
                        .help("Degree expander to use")
                        .default_value("12")
                        .takes_value(true),
                )
                .arg(
                    Arg::with_name("degree_butterfly")
                        .long("degree-butterfly")
                        .help("Degree butterfly to use")
                        .default_value("8")
                        .takes_value(true),
                )
                .arg(
                    Arg::with_name("num_expander_layers")
                        .long("num-expander-layers")
                        .help("How many expander layers to use")
                        .default_value("2")//"8")
                        .takes_value(true),
                )
                .arg(
                    Arg::with_name("num_butterfly_layers")
                        .long("num-butterfly-layers")
                        .help("How many butterfly layers to use")
                        .default_value("2")//"3")
                        .takes_value(true),
                )
                .arg(
                    Arg::with_name("hasher")
                        .long("hasher")
                        .help("Which hasher should be used. Available: \"poseidon\", \"pedersen\", \"sha256\", \"blake2s\" (default \"poseidon\")")
                ...
                .arg(
                    Arg::with_name("partitions")
                        .long("partitions")
                        .help("How many partitions to use")
                        .default_value("1")
                        .takes_value(true),
                )
                .arg(
    ...
    match matches.subcommand() {
        ...
        ("nse", Some(m)) => {
            ...
            nse::run(nse::RunOpts {
                ...
                no_tmp: m.is_present("no-tmp"),
                partitions: value_t!(m, "partitions", usize)?,
                degree_expander: value_t!(m, "degree_expander", usize)?,
                degree_butterfly: value_t!(m, "degree_butterfly", usize)?,
                num_expander_layers: value_t!(m, "num_expander_layers", usize)?,
                num_butterfly_layers: value_t!(m, "num_butterfly_layers", usize)?,
                num_challenges_window: value_t!(m, "num_challenges_window", usize)?,
                size: value_t!(m, "size", usize)?,
            })?;
        }
        ("window-post", Some(m)) => {
