...
use crate::{
    constants::{
        ...
        TreeDDomain, TreeRDomain, ALLOWED_SECTOR_SIZES,
    },
    gadgets::{
...
mod tests {
    ...
    use crate::{
        constants::{
            TreeRHasher, SECTOR_SIZE_1_KIB, SECTOR_SIZE_2_KIB, SECTOR_SIZE_4_KIB, SECTOR_SIZE_8_KIB,
            SECTOR_SIZE_16_KIB, SECTOR_SIZE_32_KIB,
        },
        encode::encode_new_replica,
        Challenges,
    };
    ...
    fn get_apex_leafs(
        ...
        tree_d_new.read_range(apex_leafs_start, apex_leafs_stop).expect(&format!(
        ...
        for k in 0..pub_params.partition_count {
            ...
            let challenge_proofs: Vec<ChallengeProof<TreeR>> =
                Challenges::new(sector_nodes, comm_r_new, k)
                    ...
                    .map(|(i, c)| {
                        let proof_r_old = tree_r_old
                            .gen_proof(c)
                            .expect(&format!("failed to generate `proof_r_old` for c_{}={}", i, c));
                        let proof_d_new = tree_d_new
                            .gen_proof(c)
                            .expect(&format!("failed to generate `proof_d_new` for c_{}={}", i, c));
                        let proof_r_new = tree_r_new
                            .gen_proof(c)
                            .expect(&format!("failed to generate `proof_r_new` for c_{}={}", i, c));
