...
impl LayerChallenges {
    ...
    fn derive_porep_synth<D: Domain>(
        ...
        SynthChallenges::default_chacha20(sector_nodes, &replica_id, &comm_r).gen_porep_partition_challenges(
    ...
    pub fn derive_synth_indexes<D: Domain>(
        ...
        let replica_id: Fr = (*replica_id).into();
        let comm_r: Fr = (*comm_r).into();
        SynthChallenges::default_chacha20(sector_nodes, &replica_id, &comm_r).gen_partition_synth_indexes(
            partition_challenge_count,
            seed,
            k as usize,
        )
...
pub mod synthetic {
    ...
    // When generating random values (via a prf) during synthetic challenge generation, a dst should
    // be used to ensure uniqueness of the generated randomness; if the prf is sha256 these bytes
    // are included in each synthetic challenge's preimage, otherwise when the prf if chacha20 these
    // bytes are the chacha20 nonce.
    const SYNTH_DST: &[u8; 12] = b"synth-porep\x00";
    const CHACHA20_EMPTY_NONCE: [u8; 12] = [0; 12];
    ...
    impl Clone for SynthChallenges {
        fn clone(&self) -> Self {
            ...
            let comm_r =
                Fr::from_repr_vartime(comm_r).expect("comm-r from repr should not fail");
            let mut synth = match self.prf {
                Prf::Sha256 => Self::new_sha256(sector_nodes, &replica_id, &comm_r, num_synth_challenges),
                Prf::ChaCha20 { .. } => {
            ...
            let challenge = match self.prf {
                Prf::Sha256 => {
                    let rand_bytes = Sha256::new()
                        ...
                        .chain_update(self.comm_r)
                        .chain_update(SYNTH_DST)
    ...
    impl SynthChallenges {
        ...
        pub fn new_chacha20(
            ...
            let key = Sha256::new()
                ...
                .chain_update(&comm_r)
                .finalize();
            let chacha20 = ChaCha20::new(&key.into(), SYNTH_DST.into());
            SynthChallenges {
                sector_nodes,
                replica_id,
                comm_r,
        ...
        pub fn gen_partition_synth_indexes(
            ...
            let rand_bytes: Vec<u8> = match self.prf {
                Prf::Sha256 => {
                ...
                Prf::ChaCha20 { .. } => {
                    let key = Sha256::new()
                        ...
                        .chain_update(rand)
                        .finalize();
                    // An empty nonce is sufficient because the porep challenge randomness is
                    // already domain-separated.
                    let mut chacha20 = ChaCha20::new(&key, &CHACHA20_EMPTY_NONCE.into());
        ...
        pub fn gen_porep_partition_challenges(
            ...
            synth_indexes.into_iter().map(|i| synth.gen_synth_challenge(i)).collect()
        }
        ...
        pub fn gen_porep_challenges(
            ...
            self.clone().gen_porep_partition_challenges(num_porep_challenges, rand, 0)
        }
    }
}
