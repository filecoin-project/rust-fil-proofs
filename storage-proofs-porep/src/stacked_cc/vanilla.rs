...
use crate::stacked::{StackedDrg, TemporaryAuxCache};
    ...
    pub fn encode_into(
        ...
        Vec::from_iter((0..end).step_by(data_block_size))
            ...
            .try_for_each(|(chunk_index, replica_data)| -> Result<()> {
                for i in (0..(chunk_size * FR_SIZE) as u64).step_by(FR_SIZE) {
        ...
        Vec::from_iter((0..end).step_by(data_block_size))
            ...
            .try_for_each(|(chunk_index, output_data)| -> Result<()> {
                for i in (0..(chunk_size * FR_SIZE) as u64).step_by(FR_SIZE) {
                    ...
                    ensure!(rho.inverse().is_some(), "Invalid randomness");
                    let out_data_fr = (replica_data_fr - sector_key_fr) * rho.inverse().unwrap();
