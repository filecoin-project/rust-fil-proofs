...
use crate::parameters::{get_post_params, get_post_verifying_key, window_post_setup_params};
use crate::types::{
...
use super::{get_partitions, partition_vanilla_proofs};
...
pub fn generate_with_vanilla<Tree: 'static + MerkleTreeTrait>(
    ...
    let partitions = get_partitions(vanilla_proofs.len(), &post_config);
...
pub fn generate<Tree: 'static + MerkleTreeTrait>(
    ...
    let vanilla_params = window_post_setup_params(&post_config);
    let partitions = get_partitions(replicas.len(), &post_config);
