...
struct SnarkProofVerifyNiParameters {
...
struct SnarkProofVerifyNiOutput {
    verifies: bool,
}
...
fn snark_proof_verify<Tree: 'static + MerkleTreeTrait>(
    ...
    let vanilla_params = SetupParams {
        ...
        // Even if the proofs come from the a non-interactive porep, it doesn't matter for the
        // SNARK, henc we can leave the features section empty.
...
fn main() -> Result<()> {
    ...
    let params: SnarkProofVerifyNiParameters = cli::parse_stdin()?;
    info!("{:?}", params);
    ...
    let output = SnarkProofVerifyNiOutput { verifies };
    info!("{:?}", output);
