...
use filecoin_proofs::{
    commitment_from_fr, get_base_tree_leafs, get_base_tree_size, DefaultOctTree, OCT_ARITY,
};
use log::{info, trace};
...
use storage_proofs_core::{
    cache_key::CacheKey, merkle::create_base_merkle_tree, util::default_rows_to_discard,
};
...
fn main() -> Result<()> {
    ...
    let base_tree_size = get_base_tree_size::<DefaultOctTree>(params.sector_size.into())?;
    let base_tree_leafs = get_base_tree_leafs::<DefaultOctTree>(base_tree_size)?;
    trace!(
    ...
    let data_tree =
        create_base_merkle_tree::<DefaultOctTree>(Some(config), base_tree_leafs, &data)?;
