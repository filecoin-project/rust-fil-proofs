...
The input parameters are given by piping in JSON with the following keys:
 - input_path: The unsealed sector that should get sealed.
 ...
 - replica_path: The path to the replica.
 - sector_size: The size of the sector in bytes.

Example JSON:
{
  "input_path": "/path/to/unsealed/sector.file",
...
read -r input_args
input_path=$(echo "${input_args}" | ${JQ} '.input_path')
...
if [ "${input_path}" = 'null' ]; then
    echo "'input_path' not set." && exit 4
fi
if [ "${output_dir}" = 'null' ]; then
    echo "'output_dir' not set." && exit 5
fi
if [ "${porep_id}" = 'null' ]; then
    echo "'porep_id' not set." && exit 6
fi
if [ "${replica_id}" = 'null' ]; then
    echo "'replica_id' not set." && exit 7
fi
if [ "${replica_path}" = 'null' ]; then
    echo "'replica_path' not set." && exit 8
fi
if [ "${sector_size}" = 'null' ]; then
    echo "'sector_size' not set." && exit 9
fi
...
# Tree building for the data commitment.
tree_d=$(${JO} input_path="${input_path}" output_dir="${output_dir}" sector_size="${sector_size}" | ${CARGO} --bin tree-d)
>&2 echo "TreeD: ${tree_d}"
...
# Encode the replica with the sector key.
encode_replica=$(${JO} input_path="${input_path}" replica_path="${replica_path}" sector_key_path="${sector_key_path}" | ${CARGO} --bin encode-replica)
>&2 echo "Encode Replica: ${encode_replica}"
