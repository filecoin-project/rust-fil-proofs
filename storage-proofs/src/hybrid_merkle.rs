...
use crate::hasher::hybrid::HybridHasher;
use crate::merkle::{MerkleProof, MerkleTree};

pub type HybridMerkleTree<AH, BH> = MerkleTree<
    <HybridHasher<AH, BH> as Hasher>::Domain,
    <HybridHasher<AH, BH> as Hasher>::Function,
>;

pub type HybridMerkleProof<AH, BH> = MerkleProof<HybridHasher<AH, BH>>;
...
//                     let left_beta = pair[0].into_beta();
//                     let right_beta = pair[1].into_beta();
//                     let child_beta =
...
mod tests {
    ...
    fn test_hybrid_merkle_tree_alpha_only() {
        ...
        let tree = HybridMerkleTree::<PedersenHasher, Blake2sHasher>::new(leaves /*, 0*/);
        assert_eq!(tree.len(), N_NODES);
    ...
    fn test_hybrid_merkle_tree_beta_only() {
        ...
        let tree =
            HybridMerkleTree::<PedersenHasher, Blake2sHasher>::new(leaves /*, HEIGHT + 1*/);
        assert_eq!(tree.len(), N_NODES);
        ...
        for challenge_index in CHALLENGES.iter() {
            let proof = tree.gen_proof(*challenge_index);
            // TODO: assert!(proof.validate(*challenge_index));
        }
    }
    ...
    fn test_hybrid_merkle_tree_seam_root() {
        let leaves = (*BETA_LEAVES).clone();
        let tree = HybridMerkleTree::<PedersenHasher, Blake2sHasher>::new(leaves /*, HEIGHT*/);
        assert_eq!(tree.len(), N_NODES);
        ...
        let expected_alpha_root = expected_nodes[N_NODES - 1].convert_into_alpha();
    ...
    fn test_hybrid_merkle_tree_with_seam() {
        ...
        let leaves = (*BETA_LEAVES).clone();
        let tree =
            HybridMerkleTree::<PedersenHasher, Blake2sHasher>::new(leaves /*, BETA_HEIGHT*/);
        assert_eq!(tree.len(), N_NODES);
