...
pub struct Tau {
    pub comm: <VectorCommitment<RsaAccumulator> as StaticVectorCommitment>::Commitment,
}
    ...
    G: 'a + Graph<H> + ParameterSetIdentifier + Sync + Send,
{
    ...
    fn replicate(
        ...
        let n = 1024;
        let lambda = 128;
        let vc = create_vector_commitment::<RsaAccumulator>(lambda, n);
        ...
        let opened = big_data.extend(&big_data_enc);
        let comm = vc.batch_open(&opened, &(0..opened.len()).iter().collect());

        Ok((Tau { comm }, ProverAux {}))
    }
...
mod tests {
    ...
    fn prove_verify_aux<H: Hasher>(
        ...
        loop {
            ...
            let real_proof = BbfVc::<H, _>::prove(&pp, &pub_inputs, &priv_inputs).unwrap();
            ...
            if use_wrong_parents {
                ...
                assert!(
                    !BbfVc::verify(&pp, &pub_inputs, &proof).unwrap(),
                ...
                assert!(
                    !BbfVc::<H, _>::verify(&pp, &pub_inputs, &proof2).unwrap(),
            ...
            if use_wrong_challenge {
                ...
                let verified =
                    BbfVc::<H, _>::verify(&pp, &pub_inputs_with_wrong_challenge_for_proof, &proof)
                        .unwrap();
                assert!(
                    !verified,
    ...
    #[test]
    fn test_bbf_vc_verifies_using_challenge() {
        prove_verify_wrong_challenge(5, 1);
    }

    #[test]
    fn test_bbf_vc_verifies_parents() {
