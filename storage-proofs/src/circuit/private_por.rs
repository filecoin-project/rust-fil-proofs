...
implement_por!(
    PrivatePoRCircuit,
    PrivatePoRCompound,
    "private-proof-of-retrievability",
    true
);
...
mod tests {
    ...
    fn test_compound() {
        ...
        for i in 0..3 {
            ...
            let public_params =
                PrivatePoRCompound::<PedersenHasher>::setup(&setup_params).expect("setup failed");
            ...
            let proof = PrivatePoRCompound::<PedersenHasher>::prove(
                &public_params,
                &public_inputs,
                &private_inputs,
            )
            ...
            let verified =
                PrivatePoRCompound::<PedersenHasher>::verify(&public_params, &public_inputs, proof)
                    .expect("failed while verifying");
            assert!(verified);

            let (circuit, inputs) = PrivatePoRCompound::<PedersenHasher>::circuit_for_test(
                &public_params,
                &public_inputs,
                &private_inputs,
            );
