    ...
    PP: ParameterSetIdentifier,
{
    ...
    fn cache_identifier(pub_params: &PP) -> Option<String> {
        ...
        debug!(SP_LOG, "parameter set identifier for cache: {}", param_identifier; "target" => "params");
    ...
    fn get_groth_params(
        ...
        let generate = || {
            debug!(SP_LOG, "Actually generating groth params."; "target" => "params");
            let start = Instant::now();
            let parameters = groth16::generate_random_parameters::<E, _, _>(circuit, rng);
            let generation_time = start.elapsed();
            debug!(SP_LOG, "groth_parameter_generation_time: {:?}", generation_time; "target" => "stats");
            parameters
        };
        ...
        match Self::cache_identifier(pub_params) {
            Some(id) => {
                ...
                debug!(SP_LOG, "checking cache_path: {:?}", cache_path; "target" => "params");
                ...
                read_cached_params(&cache_path).or_else(|_| {
                    ...
                    debug!(SP_LOG, "wrote parameters to cache {:?} ", f; "target" => "params");
                    debug!(SP_LOG, "groth_parameter_bytes: {}", bytes; "target" => "stats");
                    Ok(p)
                })
            }
            None => Ok(generate()?),
...
pub fn read_cached_params<E: JubjubEngine>(cache_path: &PathBuf) -> Result<groth16::Parameters<E>> {
    ...
    f.lock_exclusive()?;
    debug!(SP_LOG, "reading groth params from cache: {:?}", cache_path; "target" => "params");
