...
impl<Tree: MerkleTreeTrait, G: Hasher> Proof<Tree, G> {
    ...
    pub fn verify(
        ...
        graph
            ...
            .expect("parents failed"); // FIXME: error handling
    ...
    fn verify_labels(
        ...
        for layer in 1..=layer_challenges.layers() {
            ...
            let labeling_proof = &self.labeling_proofs.get(layer - 1).expect("");
            let labeled_node = self
                .replica_column_proofs
                .c_x
                .get_node_at_layer(layer)
                .expect("get_node_at_layer failed"); // FIXME: error handling
...
impl<Tree: MerkleTreeTrait, G: Hasher> TemporaryAuxCache<Tree, G> {
    pub fn new(t_aux: &TemporaryAux<Tree, G>, replica_path: PathBuf) -> Result<Self> {
        ...
        let tree_d_size = match t_aux.tree_d_config.size {
            Some(size) => size,
            None => bail!("tree_d_config size is None"),
        };
        ...
        let tree_c_size = match t_aux.tree_c_config.size {
            Some(size) => size,
            None => bail!("tree_c_config size is None"),
        };
        trace!(
        ...
        let tree_r_last_size = match t_aux.tree_r_last_config.size {
            Some(size) => size,
            None => bail!("tree_r_last_config is None"),
        };
...
impl<Tree: MerkleTreeTrait> Labels<Tree> {
    ...
    pub fn labels_for_layer(
        ...
        let size = match config.size {
            Some(size) => size,
            None => bail!("config size is None"),
        };
        DiskStore::new_from_disk(size, Tree::Arity::to_usize(), &config)
    }
        ...
        let rows = self
            ...
            .map(|label| {
                let size = match label.size {
                    Some(size) => size,
                    None => bail!("label size is None"),
                };
                let store = DiskStore::new_from_disk(size, Tree::Arity::to_usize(), &label)?;
