...
pub fn create_label(
    ...
    let hash = if node == 0 {
        ...
        if first_layer {
            let mut parents = [0u8; BASE_DEGREE * NODE_SIZE]; // FIXME: Don't hardcode 14.
            fetch(&cache_parents[0..BASE_DEGREE], layer_labels, &mut parents);

            hash_fixed_size(&mut hasher, 37, &parents);

            hasher.finalize()

        // copy_parents_data_exp(node as u32, &*layer_labels, exp_labels, hasher, cache)?
        } else {
            let mut parents = [0u8; 14 * NODE_SIZE]; // FIXME: Don't hardcode 14.
            fetch(
                &cache_parents[0..BASE_DEGREE],
                layer_labels,
                &mut parents[0..BASE_DEGREE * NODE_SIZE],
            );

            fetch(
                &cache_parents[BASE_DEGREE..],
                exp_labels,
                &mut parents[BASE_DEGREE * NODE_SIZE..],
            );
...
fn fetch(parents: &[u32], data: &[u8], buf: &mut [u8]) {
    for (i, parent) in parents.iter().enumerate() {
        let start = *parent as usize * NODE_SIZE;
        let end = start + NODE_SIZE;
        buf[32 * i..32 * (i + 1)].copy_from_slice(&data[start..end]);
    }
}

fn update_many(hasher: &mut Sha256, data: &[u8]) {
    assert!(data.len() % 32 == 0);
    let mut i = 0;
    while i < data.len() {
        hasher.update(&data[i..i + 32]);
        i += 32;
    }
}

fn hash_fixed_size(hasher: &mut Sha256, target_size: usize, data: &[u8]) {
    assert_eq!(0, data.len() % 32);
    let mut remaining = target_size;

    while remaining > 0 {
        if remaining * 32 >= data.len() {
            update_many(hasher, data);
            remaining -= data.len() / 32;
        } else {
            update_many(hasher, &data[0..remaining * 32]);
            remaining = 0;
        }
    }
}
