...
use fil_halo2_gadgets::{
    ...
    sha256::{
        compress::{self, values, BLOCK_WORDS, PAD_WORDS, STATE_WORDS},
        get_padding,
    },
...
type CompressChip = compress::CompressChip<Fp, BIT_COLS>;
type CompressConfig = compress::CompressConfig<Fp, BIT_COLS>;
...
fn sha256_chip_compute_k() {
    ...
    for num_blocks in PREIMAGE_BLOCK_SIZES {
        ...
        let expected_digest = values::hash_words_padded(&padded_preimage_words);
...
fn compress_chip_compute_k() {
    ...
    impl Circuit<Fp> for MyCircuit {
        ...
        fn synthesize(
            ...
            let digest = CompressChip::construct(config).hash_unassigned(layouter, &self.blocks)?;
    ...
    for num_blocks in PREIMAGE_BLOCK_SIZES {
        ...
        let expected_digest = values::hash_blocks_padded(&padded_preimage_blocks);
