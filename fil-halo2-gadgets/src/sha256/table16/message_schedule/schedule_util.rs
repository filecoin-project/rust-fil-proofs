...
impl<F: FieldExt> MessageScheduleConfig<F> {
    ...
    pub fn assign_word_and_halves(
        ...
        let w_lo = {
            ...
            AssignedBits::<F, 16>::assign(region, || format!("W_{}_lo", word_idx), a_3, row, w_lo_val)?
        };
        let w_hi = {
            let w_hi_val = word.map(|word| (word >> 16) as u16);
            AssignedBits::<F, 16>::assign(region, || format!("W_{}_hi", word_idx), a_4, row, w_hi_val)?
        };
    ...
    // Copies `word` into the current region and assigns its low and high 16-bit halves.
    //
    ...
    //
    // Returns an error if `word`'s column and the `a_5` column are not equality enabled.
    pub fn copy_word_and_assign_halves(
        ...
        let lo = AssignedBits::<F, 16>::assign(
            region,
        ...
        let hi = AssignedBits::<F, 16>::assign(
            region,
